...
class CustomButtonBig extends StatelessWidget {
  ...
  final double? height;
  final bool loading;
  const CustomButtonBig({super.key,
    ...
    return Material(
      ...
      child: InkWell(
        ...
        child: SizedBox(
          ...
          child: Center(child: loading ? const CircularProgressIndicator(color: background,):
          Text(btnHeading,style: btnTextStyle,)),
        ),
      ),
    );
  }
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: InkWell(
        borderRadius: BorderRadius.circular(10),
        onTap: onTap,
        child: loading ? const CircularProgressIndicator(color: greenColor,) :Container(
          height: 50,
        ...
        Material(
          ...
          child: InkWell(
            ...
            child: Container(
              ...
              child: Center(child: loading ? const CircularProgressIndicator(color: background):
              Text(btnHeading,style:  GoogleFonts.lato(
                  color: disable ? background.withOpacity(.6) : background,
                  fontSize: 16,
...
class Login_SignUpBtn extends StatelessWidget {
  ...
  final String btnHeading;
  const Login_SignUpBtn({super.key, required this.onTap, required this.sideHeading, required this.btnHeading});
